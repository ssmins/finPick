...
import CompareDepositDetailView from "./CompareDetailView.vue"; // 재사용
...
const fetchSavings = async () => {
  try {
    const response = await axios.get(
      "http://127.0.0.1:8000/products/saving-products/",
      {
        params: {
    ...
    savingProducts.value = response.data; // 데이터 업데이트
  } catch (error) {
    console.error("적금 데이터 가져오기 실패:", error);
  }
};

// 필터 적용 함수
const applyFilter = () => {
  fetchSavings(); // 선택된 필터로 데이터 재조회
};
...
<template>
  <div>
    ...
    <!-- 적금 상품 목록 -->
    <div>
      <h2 class="list-title">정기 적금 상품 목록</h2>
      <table>
        <thead>
          <tr>
            <th>공시 제출월</th>
